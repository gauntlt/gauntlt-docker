...
def main(argv):
    ...
    if running_in_docker():
        ...
        try:
            ...
            try:
                subprocess.check_output(['docker', 'exec', '-t', cid, 'mkdir', '-p', '/opt/zap/scripts/scripts/httpsender/'])
                cp_to_docker(cid, 'scripts/scripts/httpsender/Alert_on_HTTP_Response_Code_Errors.js', '/opt/zap/')
                cp_to_docker(cid, 'scripts/scripts/httpsender/Alert_on_Unexpected_Content_Types.js', '/opt/zap/')
                cp_to_docker(cid, 'policies/API-Minimal.policy', '/opt/zap/')
                if target_file:
    ...
    try:
        ...
        zap.script.load('Alert_on_Unexpected_Content_Types.js', 'httpsender', 'Oracle Nashorn', '/opt/zap/scripts/scripts/httpsender/Alert_on_Unexpected_Content_Types.js')
